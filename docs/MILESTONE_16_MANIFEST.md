...
- `backend/app/core/constants.py`:
  ...
  - Added enums `ThresholdStatus` (`ABOVE_THRESHOLD`, `NEAR_THRESHOLD`, `BELOW_THRESHOLD`), `ExportFormat` (`CSV`, `XLSX`), `ImportType`, `ImportJobStatus`, `ImportRowStatus`, and `ImportCommitMode`.
...
- `backend/app/imports/processors/`:
  ...
  - `students.py`: Student and User provisioning with role assignments.
  - `lecturers.py`: Lecturer and User provisioning with academic unit validation.
...
- `backend/app/reports/schemas.py`: Pydantic contracts for Course Roster, Student Summary, Session Operational Stats, Department, Faculty, and Lecturer reports.
- `backend/app/reports/exports.py`: CSV and XLSX export generator featuring formula injection defense (`=`, `+`, `-`, `@`, `\t`, `\r` escaping) and Excel UTF-8 BOM compatibility.
- `backend/app/reports/service.py`: Authoritative reporting service calculating dynamic attendance percentage, decimal-safe threshold evaluation, and subtree aggregations.
...
- `apps/web/src/app/admin/reports/attendance/page.tsx`: Attendance Reports Dashboard providing offering selection, threshold KPI cards, student attendance breakdown table, and CSV/XLSX export triggers.
...
- `backend/tests/test_student_import.py`: 6 tests for student preview, strict commit, partial commit, duplicate updates, and cancellation.
...
- `backend/tests/test_attendance_thresholds.py`: 3 tests for neutral threshold states, decimal-safe boundaries, and zero-denominator safety.
...
- `backend/tests/test_import_hardening.py`: 4 tests for atomicity in STRICT mode, commit idempotency, frozen history preservation, and 3,000-student performance benchmark.
...
## 3. Known Limitations & Milestone 17 Scope
