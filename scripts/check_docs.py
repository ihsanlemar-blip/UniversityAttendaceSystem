...
EXPECTED_PREFIXES = [f"{i:02d}" for i in range(41)]
...
def validate_docs() -> int:
    ...
    print(
        f"[+] Documentation validation PASSED: All 41 specifications (00-40), "
        f"and manifests (M3, M4, M5, M6, M7, M8, M9) present ({len(files)} total files)."
    )
    return 0
