...
EXPECTED_PREFIXES = [f"{i:02d}" for i in range(39)]
...
def validate_docs() -> int:
    ...
    print(
        f"[+] Documentation validation PASSED: All 39 specifications (00-38), "
        f"and manifests (M3, M4, M5, M6, M7, M8) present ({len(files)} total files)."
    )
    return 0
